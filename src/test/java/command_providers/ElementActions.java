...
public class ElementActions {
    ...
    public WebElement getElement() {
        // Implicit Wait is injected one time into the method WebElement and will now wait 15 seconds to find each element by locator
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(15));
        WebElement element = null;
        try {
            element = driver.findElement(locator);
        } catch (Exception e) {
            LOGGER.error("Element exceptions for the locator: " + locator + " and exception is: " + e.getMessage());
        }
        return element;
    }
